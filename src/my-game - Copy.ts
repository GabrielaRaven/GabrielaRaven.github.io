...
class Bomb {
    ...
    plant() {
        ...
        let tile_under_bomb = getSpriteAtPos(this.origin[_X], this.origin[_Y], Tags.PATH, this.scene.scene);
        tile_under_bomb.addTag(Tags.DANGER);
        tile_under_bomb.removeTag(Tags.PATH);
        this.danger_sprites.push(tile_under_bomb.asSprite());
...
class AIController extends PlayerController {
    ...
    onUpdate(delta: number, absolute: number) {
        ...
        if (!this.isSafe()) {
            ...
            if (delta % 20 < 10) {this.getSafeTile();}
    ...
    isSafe() {
        return !this.owner.hasFlag(PlayerFlags.IN_DANGER);
    }

    getSafeTile() {
        let path_layer = this.owner.getChildByName('pathLayer') as ECS.Container;
